...
* =============================================================================
*/
let Ld=null;function Rd(e){self.postMessage({type:`progress`,...e})}function zd(e,t,n){self.postMessage({type:`done`,vocals:e,instrumental:t,sampleRate:n},{transfer:[e.buffer,t.buffer]})}function Bd(e){self.postMessage({type:`error`,message:e})}function Vd(){self.postMessage({type:`modelLoaded`})}function Hd(e,t){let n=e.length;if(n<=1)return;let r=0;for(let i=1;i<n;i++){let a=n>>1;for(;r&a;a>>=1)r^=a;if(r^=a,i<r){let n=e[i];e[i]=e[r],e[r]=n,n=t[i],t[i]=t[r],t[r]=n}}for(let r=2;r<=n;r<<=1){let i=r>>1,a=-2*Math.PI/r;for(let o=0;o<n;o+=r){let n=0;for(let r=0;r<i;r++){let s=Math.cos(n),c=Math.sin(n),l=e[o+r+i]*s-t[o+r+i]*c,u=e[o+r+i]*c+t[o+r+i]*s;e[o+r+i]=e[o+r]-l,t[o+r+i]=t[o+r]-u,e[o+r]+=l,t[o+r]+=u,n+=a}}}}function Ud(e,t){let n=e.length;for(let e=0;e<n;e++)t[e]=-t[e];Hd(e,t);for(let r=0;r<n;r++)e[r]/=n,t[r]=-t[r]/n}function Wd(e,t,n){let r=Math.floor((e.length-t)/n)+1,i=t/2+1,a=new Float32Array(r*i*2),o=new Float32Array(t);for(let e=0;e<t;e++)o[e]=.5*(1-Math.cos(2*Math.PI*e/t));let s=new Float32Array(t),c=new Float32Array(t);for(let l=0;l<r;l++){let r=l*n,u=l*i*2;for(let n=0;n<t;n++)s[n]=e[r+n]*o[n],c[n]=0;Hd(s,c);for(let e=0;e<i;e++)a[u+e*2]=s[e],a[u+e*2+1]=c[e]}return{stftData:a,nBins:i,nFrames:r}}function Gd(e,t,n,r,i,a,o){let s=(o-1)*r+n,c=new Float32Array(s),l=new Float32Array(s),u=new Float32Array(n);for(let e=0;e<n;e++)u[e]=.5*(1-Math.cos(2*Math.PI*e/n));let d=new Float32Array(n),f=new Float32Array(n);for(let i=0;i<o;i++){let o=i*a*2,s=i*a;for(let n=0;n<a;n++){let r=t[o+n*2],i=t[o+n*2+1],a=Math.sqrt(r*r+i*i)||1,c=r/a,l=i/a,u=e[s+n];d[n]=u*c,f[n]=u*l}for(let e=1;e<a-1;e++)d[n-e]=d[e],f[n-e]=-f[e];Ud(d,f);let p=i*r;for(let e=0;e<n;e++)c[p+e]+=d[e]*u[e],l[p+e]+=u[e]*u[e]}for(let e=0;e<s;e++)l[e]>1e-10&&(c[e]/=l[e]);let p=new Float32Array(i);return p.set(c.subarray(0,Math.min(i,s))),p}function Kd(e){if(e.numberOfChannels===1)return new Float32Array(e.getChannelData(0));let t=new Float32Array(e.length),n=e.getChannelData(0),r=e.getChannelData(1);for(let i=0;i<e.length;i++)t[i]=(n[i]+r[i])*.5;return t}var qd=class{session=null;sampleRate=44100;windowSize=2048;hopSize=512;chunkFrames=512;modelUrl=`https://huggingface.co/Anjok07/ultimatevocalremovergui/resolve/main/model/MDX-Net_Model1.onnx`;async loadModel(){if(!this.session){Rd({stage:`loading`,percent:5,message:`正在加载 AI 分离模型...`});try{x.wasm.numThreads=Math.min(navigator.hardwareConcurrency||4,8),x.wasm.simd=!0,x.wasm.trace=!1,x.allowLocalModelsOnly=!1,Rd({stage:`loading`,percent:30,message:`正在下载模型文件...`}),this.session=await Se.create(this.modelUrl,{executionProviders:[`wasm`],graphOptimizationLevel:`all`,executionMode:`parallel`,enableMemPattern:!0,enableCpuMemArena:!0}),Rd({stage:`loading`,percent:85,message:`模型加载完成，预热中...`});try{let e=new fe(`float32`,new Float32Array(32800),[1,1,1025,32]);await this.session.run({[this.session.inputNames[0]]:e})}catch{}Rd({stage:`loading`,percent:100,message:`模型加载完成，准备就绪`}),Vd()}catch{this.session=null,Rd({stage:`loading`,percent:50,message:`模型加载较慢，使用备用算法...`})}}}async separate(e,t,n){let r={numberOfChannels:e.length,length:n,sampleRate:t,getChannelData:t=>e[t]};Rd({stage:`analyzing`,percent:5,message:`正在分析音频...`});let i=Kd(r),a=this.windowSize,o=this.hopSize;Rd({stage:`analyzing`,percent:15,message:`正在计算频谱...`});let{stftData:s,nBins:c,nFrames:l}=Wd(i,a,o);Rd({stage:`separating`,percent:25,message:`正在分离人声与伴奏...`});let u,d;if(this.session){let e=await this.separateWithModel(s,c,l);u=e.vocalsMag,d=e.instrumentalMag}else{let e=this.separateSpectral(s,c,l);u=e.vocalsMag,d=e.instrumentalMag}Rd({stage:`separating`,percent:88,message:`正在重建时域波形...`});let f=Gd(u,s,a,o,i.length,c,l),p=Gd(d,s,a,o,i.length,c,l);Rd({stage:`done`,percent:100,message:`分离完成！`}),zd(f,p,t)}async separateWithModel(e,t,n){let r=new Float32Array(t*n),i=0;for(let a=0;a<n;a++){let n=a*t*2,o=a*t;for(let a=0;a<t;a++){let t=e[n+a*2],s=e[n+a*2+1],c=Math.sqrt(t*t+s*s);r[o+a]=c,c>i&&(i=c)}}let a=i>0?1/i:1,o=this.chunkFrames,s=Math.ceil(n/o),c=new Float32Array(t*n),l=new Float32Array(t*n),u=new Float32Array(1*t*o),d=new fe(`float32`,u,[1,1,t,o]);for(let e=0;e<s;e++){let i=e*o,f=Math.min(i+o,n)-i;f<o&&u.fill(0);for(let e=0;e<f;e++){let n=(i+e)*t,s=e;for(let e=0;e<t;e++)u[e*o+s]=r[n+e]*a}try{let e=(await this.session.run({[this.session.inputNames[0]]:d}))[this.session.outputNames[0]].data;for(let n=0;n<f;n++){let a=(i+n)*t,s=n;for(let n=0;n<t;n++){let i=n*o+s,u=e[i],d=e[o*t+i],f=r[a+n];c[a+n]=f*(u>0?u<1?u:1:0),l[a+n]=f*(d>0?d<1?d:1:0)}}}catch{for(let e=0;e<f;e++){let n=(i+e)*t;for(let e=0;e<t;e++){let t=r[n+e];c[n+e]=t*.3,l[n+e]=t*.7}}}Rd({stage:`separating`,percent:25+(e+1)/s*.6*100,message:`AI 分离中... ${Math.round(25+(e+1)/s*.6*100)}%`})}return{vocalsMag:c,instrumentalMag:l}}separateSpectral(e,t,n){let r=new Float32Array(t*n);for(let i=0;i<n;i++){let n=i*t*2,a=i*t;for(let i=0;i<t;i++){let t=e[n+i*2],o=e[n+i*2+1];r[a+i]=Math.sqrt(t*t+o*o)}}let i=new Float32Array(t*n),a=new Float32Array(t*n),o=this.sampleRate,s=Math.max(1,Math.floor(80*t*2/o)),c=Math.min(t-1,Math.ceil(1100*t*2/o)),l=new Float32Array(n),u=new Float32Array(n);for(let e=0;e<n;e++){let n=e*t,i=0,a=0,o=0;for(let e=0;e<t;e++){let t=r[n+e],o=t*t;i+=o,a+=e*o}if(i>1e-10){let s=a/i;l[e]=s;for(let e=0;e<t;e++){let t=r[n+e],i=e-s;o+=i*i*t*t}u[e]=Math.sqrt(o/i)}}for(let e=0;e<n;e++){let o=e*t,d=l[e],f=u[e],p=f>0?Math.max(0,1-f/(t*.15)):0,m=d>=s&&d<=c?1-Math.abs(d-(s+c)/2)/((c-s)/2)*.5:.2,h=Math.min(1,p*.6+m*.4+.1);for(let e=0;e<t;e++){let t=o+e,n=r[t],l=e>=s&&e<=c?h:h*.4,u=Math.max(.1,Math.min(.9,l));i[t]=n*u,a[t]=n*(1-u)}e%100==0&&Rd({stage:`separating`,percent:25+e/n*.6*100,message:`信号处理分离中... ${Math.round(25+e/n*.6*100)}%`})}return{vocalsMag:i,instrumentalMag:a}}};self.onmessage=async e=>{let{action:t,payload:n}=e.data;switch(t){case`loadModel`:Ld||=new qd;try{await Ld.loadModel()}catch(e){Bd(`模型加载失败：`+String(e))}break;case`separate`:Ld||=new qd;try{let{channelData:e,sampleRate:t,length:r}=n;await Ld.separate(e,t,r)}catch(e){Bd(`分离失败：`+String(e))}}}})();